...
export default class SoundHelper {
  ...
  static async speak(words: string, gender: string, locale: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      ...
      fetchWithTimeout(path, {}, SERVER_TIMEOUT)
        .then((res) => {
          if (res.status !== 200) {
        ...
        .then((decodedBuffer) => {
          ...
          const maxDuration = Math.max(extendedBuffer.duration, processedDuration) // Maximum of original and processed durations
          ...
          const pitchShifter = new PitchShifter(offlineContext, extendedBuffer, 4096)
          ...
          offlineContext
            ...
            .then((renderedBuffer) => {
              ...
              const mp3SoundBuffers = this.convertSoundToMP3(renderedBuffer)
              const mp3SoundData = this.convertMp3BufferToData(mp3SoundBuffers)
