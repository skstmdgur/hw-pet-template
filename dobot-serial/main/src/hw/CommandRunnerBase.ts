...
export class CommandRunnerBase implements IHPetCommandRunner {
  ...
  protected write_ = async (
    ...
    const packetLength = values.length + 3
    ...
    // Calculate checksum as two's complement
    message[message.length - 1] = (~message.reduce((sum, byte) => sum + byte, 0) + 1) & 0xff
    console.log('Message with checksum:', message)

    this.dataToDevice$.next({
      requestId,
      data: message,
      debugCmd: `CmdID ${commandId}`,
    })
    ...
    try {
      const result = await firstValueFrom(
        this.dataFromDevice$.pipe(
          ...
          catchError((err) => {
            console.error('Response timeout or error:', err)
            return of(null as Uint8Array | null) // Explicitly casting to the correct type
          }),
        ),
      )
      ...
      if (!this.stopped && afterDelayMillis > 0) {
        console.log(`Command ${commandId}: \t\tsleep ${afterDelayMillis}ms`)
        await sleepAsync(afterDelayMillis)
      }

      return result
    } catch (err) {
      console.log('Error in write_:', err)
