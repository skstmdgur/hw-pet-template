export class PingPongUtill {
  //connect
  getOrangeForSoundData() {
  ...
  getSetMultiroleInAction(cubeNum) {
  ...
  makeSingleStep(cubeNum, cubeID, speed, step) {
    ...
    data[11] = 0x00
    data[12] = 0x02
    data[13] = this.intToByte(speed)[0]
    data[14] = this.intToByte(speed)[1]
    data[15] = 0x00
    data[16] = 0x00
    data[17] = this.intToByte(step)[0]
    data[18] = this.intToByte(step)[1]

    return data
  }

  makeContinuousStep(cubeNum, cubeID, speed) {
  ...
  makeAggregateStep(cubeNum, innerData, method) {
    ...
    data[7] = this.intToByte(packetSize)[0]
    data[8] = this.intToByte(packetSize)[1]
  ...
  // torque : 0(min) ~ 1(max)
  setInstantTorque = (cubeNum, torque): Uint8Array => {
  ...
  byteToString = (data: Uint8Array): String => {
    let hexStr = ''
    const hexSpace = ' '
  ...
  stringToByte = (packet: string): Uint8Array => {
  ...
  intToByte = (int: number): Uint8Array => {
  ...
  // Speed : 0 ~ 100
  // Sps : 100 ~ 1000
  changeSpeedToSps = (speed: number): number => {
