javascriptGenerator.forBlock['oz71_autocar_motor_countinous_solo'] = function (block, generator) {
  const dropdown_cube = block.getFieldValue('CUBE')
  const dropdown_direction = block.getFieldValue('DIRECTION')
  const number_speed = block.getFieldValue('SPEED')

  let speed0 = number_speed
  ...
  if (dropdown_cube === 'left') {
    ...
    if (dropdown_direction === 'front') {
      speed0 = number_speed * -1
    } else if (dropdown_direction === 'back') {
      speed1 = number_speed * -1
    }
